...
export const ViewLogisticDecrease = ({ initialPayments = [] }) => {
  ...
  const cards = [
    { title: 'Notas fiscal', status: 'emitido', color: 'primary' },
    { title: 'Conhecimentos', status: 'pendente', color: 'warning.main' },
    { title: 'Serviços', status: 'cancelado', color: 'error' },
  ];
    ...
    <>
      ...
      <Box
        ...
        sx={{
          ...
          gap: 2,
          pl: 6, // espaço para seta esquerda
          pr: 6, // espaço para seta direita
        }}
      >
        {cards.map((card, index) => {
          const count = installments.response?.rows.filter((p) => p.status === card.status).length || 0;
