...
import { ViewService } from './view.partner'

import _ from 'lodash'

export const ViewRegisterPartners = ({ initialPartners = [] }) => {
  ...
  useEffect(() => {
    setTitle(['Cadastros', 'Clientes'])
  }, [])

  const fetchServices = async (request) => {
    try {
      setIsFetching(true)
      const response = await partners.findAll(request)
      setData(response)
  ...
  return (
    <Box sx={styles.container}>
                ...
                _.map(data.response?.rows, (service, index) => {
                  ...
                  return (
                    <TableRow
                      key={index}
                      hover
                      onDoubleClick={() => setServiceId(service.codigo_pessoa)}
                    ...
                    >
                      ...
                      <TableCell>{service.cpfCnpj}</TableCell>
                      <TableCell>{service.name}</TableCell>
                      <TableCell>{service.surname}</TableCell>
      ...
      <ViewService partnerId={serviceId} onClose={(service) => {
