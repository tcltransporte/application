...
export async function findAll({limit = 50, offset}) {
    ...
    const services = await db.Partner.findAndCountAll({
        attributes: ['codigo_pessoa', 'cpfCnpj', 'name', 'surname'],
        limit: limit,
        offset: offset * limit,
        order: [['name', 'asc']],
        where
...
export async function upsert(values) {
  ...
  return await db.transaction(async (transaction) => {
    ...
    if (!partner.codigo_pessoa) {
      
      result = await db.Partner.create({ ...partner }, { transaction })

    } else {
      
      await db.Partner.update(
        { ...partner },
        { where: { codigo_pessoa: partner.codigo_pessoa }, transaction }
      )
