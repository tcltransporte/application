...
public class PamcardClient
{
    ...
    public async Task<List<KeyValuePair<string, string>>> CallExecuteAsync(X509Certificate2 cert, string? context, List<KeyValuePair<string, string>>? fields)
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Post, "https://www.roadcard.com.br/sistemapamcard/services/WSTransacional");
        request.Headers.Add("SOAPAction", "");
        request.Content = new StringContent(soapEnvelope.ToString(), Encoding.UTF8, "text/xml");
        ...
        return ParseResponseFields(content);
    }

    private List<KeyValuePair<string, string>> ParseResponseFields(string soapXml)
    {
        var result = new List<KeyValuePair<string, string>>();

        var xml = XDocument.Parse(soapXml);

        XNamespace ns = "http://webservice.pamcard.jee.pamcary.com.br";

        var fields = xml.Descendants(ns + "fields");

        foreach (var field in fields)
        {
            var key = field.Element(ns + "key")?.Value ?? "";
            var value = field.Element(ns + "value")?.Value ?? "";
